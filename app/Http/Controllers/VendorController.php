...
class VendorController extends Controller
{
    ...
    public function exportAnalytics(Request $request)
    {
        ...
        $productPerformance = Product::where('vendor_id', $vendor->id)
            ...
            ->withCount(['orderItems as total_revenue' => function ($q) {
        ...
        $styleHeader = function ($sheet, $cols, $row = 1) use ($headerFill, $headerFont) {
    ...
    public function updateOrderStatus(Request $request, $id)
    {
        ...
        $allDelivered = $allItems->every(function ($i) {
            return $i->status === 'delivered';
        });
        $allShippedOrDelivered = $allItems->every(function ($i) {
            return in_array($i->status, ['shipped', 'delivered']);
        });
    ...
    public function requestPayout(Request $request, PaystackTransferService $paystackService)
    {
        ...
        try {
            ...
            if (empty($bankCode)) {
                ...
                if (! $bankCode) {
                    throw new \Exception('Bank code could not be resolved. Please update your bank details.');
                }
            }
