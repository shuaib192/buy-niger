{{-- 
    ...
    View: Customer Messages Details (Premium v2.0)
--}}
@extends('layouts.app')
    ...
    <div class="chat-wrapper">
        ...
        <div class="chat-header-premium">
            <div class="d-flex align-items-center gap-3">
                <a href="{{ route('customer.messages.index') }}" class="back-btn" title="Back to Messages">
                ...
                <div>
                    <h4 class="chat-user-name">{{ $conversation->vendor->store_name ?? 'Vendor' }}</h4>
                    <span class="chat-subject">{{ $conversation->subject ?? 'Product Query' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="chat-messages" id="chatBox">
            @if($conversation->messages->count() == 0)
                <div class="chat-empty">
                    ...
                    <p>No messages in this conversation yet. Send a message to start.</p>
                </div>
            @endif
            @foreach($conversation->messages as $message)
                <div class="msg-row {{ $message->sender_type == 'customer' ? 'sent' : 'received' }}">
        ...
        <div class="chat-input-bar">
            <form action="{{ route('customer.messages.send', $conversation->id) }}" method="POST" id="messageForm" class="chat-form">
                @csrf
                <div class="input-container">
                    <input type="text" name="body" class="msg-input" placeholder="Type your message..." required autofocus autocomplete="off">
...
<style>
    .chat-page { animation: fadeInUp 0.35s ease; max-width: 860px; margin: 0 auto; }
    @keyframes fadeInUp { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: translateY(0); } }

    .chat-wrapper { background: white; border: 1px solid #f1f5f9; border-radius: 24px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.04); display: flex; flex-direction: column; height: calc(100vh - 200px); min-height: 500px; max-height: 750px; }
    ...
    .chat-avatar { width: 42px; height: 42px; border-radius: 14px; background: linear-gradient(135deg, #6366f1, #8b5cf6); color: white; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 16px; flex-shrink: 0; box-shadow: 0 4px 10px rgba(99,102,241,0.2); }
    ...
    .chat-empty i { font-size: 40px; margin-bottom: 12px; display: block; color: #6366f1; }
    ...
    .input-container:focus-within { background: white; border-color: #6366f1; box-shadow: 0 0 0 4px rgba(99,102,241,0.1); }
    ...
    .send-btn { width: 44px; height: 44px; border-radius: 50%; background: linear-gradient(135deg, #6366f1, #8b5cf6); color: white; border: none; display: flex; align-items: center; justify-content: center; cursor: pointer; flex-shrink: 0; transition: all 0.2s; box-shadow: 0 4px 12px rgba(99,102,241,0.25); font-size: 15px; }
    .send-btn:hover { transform: scale(1.05); box-shadow: 0 6px 18px rgba(99,102,241,0.35); }

    @media (max-width: 768px) {
        .chat-wrapper { border-radius: 16px; height: calc(100vh - 160px); }
